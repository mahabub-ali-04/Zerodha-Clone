...
app.get("/seedHoldings", async (req, res) => {
  ...
  res.json({ message: "Holdings seeded", count: tempHoldings.length });
});

// Seed Positions (run once)
app.get("/seedPositions", async (req, res) => {
  ...
  res.json({ message: "Positions seeded", count: tempPositions.length });
});
